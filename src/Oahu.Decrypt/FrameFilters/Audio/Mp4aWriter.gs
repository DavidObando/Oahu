...
open class Mp4aWriter : IDisposable {
    ...
    init(outputFile Stream, ftyp FtypBox, moov MoovBox) {
        ...
        ArgumentNullException.ThrowIfNull(ftyp, "ftyp")
        ArgumentNullException.ThrowIfNull(moov, "moov")
        if !outputFile.CanWrite {
            throw ArgumentException("The stream is not writable", "outputFile")
        }
    ...
    convenience init(outputFile Stream, ftyp FtypBox, moov MoovBox, ascBytes[]uint8) {
        ...
        ArgumentNullException.ThrowIfNull(ascBytes, "ascBytes")
        ...
        if asc.ChannelConfiguration > 2 {
            throw NotSupportedException(
                "Only supports maximum of 2-channel audio. (Channels=${asc.ChannelConfiguration})"
            )
        }
    ...
    func Close() {
        ...
        Debug.Assert(
            int64(audioSampleSizes.Count) == stts.Samples.Sum(
                func (s SttsBox.SampleEntry) int64 {
                    return s.FrameCount
                }
            )
        )
        ...
        let (maxBitRate, avgBitrate) = CalculateBitrate(
    ...
    func WriteChapter(entry ChapterEntry) {
        ...
        if !Moov.TextTrack!!.Mdia.Minf.Stbl.Stsc.Samples.Any() {
            Moov.TextTrack!!
                ...
                .Stsc
                .Samples
                .Add(StscBox.StscChunkEntry{FirstChunk: 1, SamplesPerChunk: 1, SampleDescriptionIndex: 1})
        }
        ...
        Moov.TextTrack!!
            ...
            .Stts
            .Samples
            .Add(SttsBox.SampleEntry{FrameCount: 1, FrameDelta: entry.SamplesInFrame})
    ...
    private func WriteChapterMetadata(chapterTitles IEnumerable[string]) {
        ...
        let chapterNames AppleListBox? = Moov
            .TextTrack
            ?.GetChild[UdtaBox]()
            ?.GetChild[MetaBox]()
    ...
    shared {
        private func CalculateBitrate(timeScale float64, duration uint64, stsz IStszBox, stts SttsBox)(
            MaxOneSecondBitrate uint32,
            AvgBitrate uint32
        ) {
            ...
            if int64(stts.SampleTimeCount) != int64(stsz.SampleCount) ||
                int64(stts.SampleTimeCount) != int64(frameDeltas.Length) {
                throw InvalidOperationException(
                    "The number of sample deltas (${stts.SampleTimeCount}) doesn't match the number of sample sizes (${stsz.SampleCount})."
                )
            }
            ...
            var maxOneSecondBitrate float64 = 0.0
            {
                ...
                while i < stsz.SampleCount {
                    while float64(currentWindowSampleSpan) > timeScale {
                        let bitrate = float64(windowSizeInBytes * int64(8)) * timeScale / float64(
                            currentWindowSampleSpan
                        )
        ...
        private func MakeBlankMoov(moov MoovBox) MoovBox {
            ...
            newMoov
                ...
                .Mdia
                .Mdhd
                .CreationTime = (newMoov.AudioTrack.Mdia.Mdhd.ModificationTime = DateTimeOffset.UtcNow)
