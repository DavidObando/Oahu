...
open class Mp4Operation {
    ...
    event ConversionProgressUpdate EventHandler[ConversionProgressEventArgs]
    ...
    prop IsCompletedSuccessfully bool -> readerTask?.IsCompletedSuccessfully == true &&
        Continuation?.IsCompletedSuccessfully == true
    ...
    func Start() {
        if readerTask == nil {
            readerTask = Task.Run(
                func () Task? {
                    return startAction(cancellationSource)
                }
            )
            ...
            (t Task) -> {
                ...
                try {
                    ...
                    if t.Exception != nil {
                        throw AggregateException(
                            "Two or more errors occurred.",
                            t.Exception!!.InnerExceptions.Append(ex)
                        )
                    }
                    rethrow
                }
                ...
                if t.IsCanceled {
                    throw TaskCanceledException(
                        "The decryption operation was cancelled.",
                        nil,
    ...
    shared {
        func FromCompleted(mp4File Mp4File?) Mp4Operation -> Mp4Operation(
