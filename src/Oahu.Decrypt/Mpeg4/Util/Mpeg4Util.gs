    ...
    shared {
        func LoadTopLevelBoxes(file Stream) List[IBox] {
            ...
            while file.Position < file.Length &&
                (!boxes.OfType[FtypBox]().Any() || !boxes.OfType[MoovBox]().Any() || !boxes.OfType[MdatBox]().Any()) {
        ...
        async func ShiftDataBlock(
            ...
            if start + shiftVector < int64(0) {
                throw ArgumentOutOfRangeException("shiftVector", "Data cannot be shifted to a negative file position.")
            }
            if !file.CanRead || !file.CanWrite || !file.CanSeek {
                throw ArgumentException("Stream must support reading, writing, and seeking.", "file")
            }
            if start > file.Length {
                throw ArgumentOutOfRangeException("start", "Start index exceeds the file length.")
            }
            if start + length > file.Length {
                throw ArgumentOutOfRangeException("length", "End of data block is beyond the end of the file.")
            }
            let backToFront = shiftVector > int64(0)
            file.Position = if backToFront {
                start + length
            } else {
                start
            }
...
class ProgressTracker {
    ...
    private var nextUpdate DateTime = default(DateTime)
    private var movedBytes int64
    event ProgressUpdated EventHandler
