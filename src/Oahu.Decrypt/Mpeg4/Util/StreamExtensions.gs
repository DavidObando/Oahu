...
/// @param chunkOffset The chunk's file offset
async func (inputStream Stream) SeekToOffsetAsync(chunkOffset int64, token CancellationToken) {
    if inputStream.Position == chunkOffset {
        ...
        throw NotSupportedException(
            "Input stream position 0x${inputStream.Position:X8} is past the chunk offset 0x${chunkOffset:X8} and is not seekable."
        )
    }
}
...
/// @param chunkOffset The chunk's file offset
/// @param chunkBuffer Buffer to copy the chink data into
async func (inputStream Stream) ReadNextChunkAsync(
    chunkOffset int64,
    chunkBuffer Memory[uint8],
    ...
    if await inputStream.ReadAsync(chunkBuffer, token) != chunkBuffer.Length {
        throw EndOfStreamException(
            "Stream ended at position ${inputStream.Position} before all ${chunkBuffer.Length} bytes were read."
        )
    }
}

func (stream Stream) WriteType(type string) {
    if type?.Length != 4 {
        throw ArgumentException("Type must be 4 chars long.")
    }
    stream.Write([]uint8{uint8(type[0]), uint8(type[1]), uint8(type[2]), uint8(type[3])})
}
...
func (stream Stream) ReadBlock(length int32)[]uint8 {
    if length < 0 {
        throw ArgumentException("Length must be non-negative", "length")
    } else if length == 0 {
        return []uint8{}
    } else {
