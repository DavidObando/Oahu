...
open data class ChapterInfo : IEnumerable[Chapter] {
    ...
    prop EndOffset TimeSpan -> if Count == 0 {
        StartOffset
    } else {
        chapterList.Max((c Chapter) -> c.EndOffset)
    }
    ...
    func AddChapter(title string, duration TimeSpan) {
        let startTime = if Count == 0 {
            StartOffset
        } else {
            chapterList[^1].EndOffset
        }
