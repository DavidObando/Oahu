    ...
    init(file Stream, header BoxHeader, parent IBox?) : base(file, header, parent) {
        ...
        if entryCount > uint32(int32.MaxValue) {
            throw NotSupportedException(
                "Oahu.Decrypt.Mpeg4 does not support MPEG-4 files with more than ${int32.MaxValue} chunk offsets"
            )
        }
        ChunkOffsets = ChunkOffsetList.Read32(file, entryCount)
    }

    private init(chunkOffsets ChunkOffsetList, versionFlags[]uint8, header BoxHeader, parent IBox) : base(
        versionFlags,
        header,
        parent
    ) {
