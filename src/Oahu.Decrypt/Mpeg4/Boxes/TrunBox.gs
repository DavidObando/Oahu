...
open class TrunBox : FullBox {
    init(file Stream, header BoxHeader, parent IBox?) : base(file, header, parent) {
        ...
        i++ {
            ...
            let sampleSize int32? = if SampleSizePresent {
                file.ReadInt32BE()
            } else {
                nil
            }
            let sampleFlags uint32? = if SampleFlagsPresent {
                file.ReadUInt32BE()
            } else {
                nil
            }
            let sampleCompositionTimeOffset int32? = if SampleCompositionTimeOffsetsPresent {
                file.ReadInt32BE()
            } else {
                nil
            }
    ...
    open override prop RenderSize int64 -> base.RenderSize +
        ...
        int64(SampleInfoSize * Samples.Length)
    ...
    private prop SampleInfoSize int32 -> (
    ...
    class SampleInfo {
        init(sampleDuration uint32?, sampleSize int32?, sampleFlags uint32?, sampleCompositionTimeOffset int32?) {
        ...
        prop SampleFlags uint32? {
            get;
            init;
        }

        prop SampleCompositionTimeOffset int32? {
            get;
            init;
        }
    }
