...
open class AppleDataBox : Box {
    ...
    private init(header BoxHeader, parent IBox, data[]uint8, type AppleDataType) : base(header, parent) {
        DataType = type
        Flags = uint32(0)
        Data = data
    }
    ...
    @DebuggerHidden
    private prop DebuggerDisplay string -> if DataType is AppleDataType {
        "[UTF-8]: '${Encoding.UTF8.GetString(Data)}'"
    } else {
        (
            if DataType is AppleDataType {
                "[UTF-16]: '${Encoding.Unicode.GetString(Data)}'"
            } else {
    ...
    shared {
        func Create(parent IBox, data[]uint8, type AppleDataType) AppleDataBox {
            let size = data.Length + 8
            let header = BoxHeader(uint32(size), "data")
            let dataBox = AppleDataBox(header, parent, data, type)
