...
open class VisualSampleEntry : SampleEntry {
    ...
    init(file Stream, header BoxHeader, parent IBox?) : base(file, header, parent) {
        ...
        CompressorName = Encoding.UTF8.GetString(compressorNameBytes, 1, displaySize)
        Depth = file.ReadUInt16BE()
        preDefined3 = file.ReadBlock(2)
    }

    open override prop RenderSize int64 -> base.RenderSize +
        int64(preDefined1.Length) +
        int64(reserved.Length) +
        int64(preDefined2.Length) +
        int64(2 * 4) +
        int64(4 * 2) +
        int64(reserved2.Length) +
        int64(preDefined3.Length) +
        int64(32)
    ...
    protected open override func Render(file Stream) {
        ...
        Encoding.UTF8.GetBytes(CompressorName, 0, CompressorName.Length, compressorNameBytes, 1)
