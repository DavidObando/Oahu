...
open class StscBox : FullBox {
    ...
    /// Effectively expand the Stsc table to one entry per chunk. Table size is 8 * [`numChunks`](paramref)
    /// bytes.
    /// @param numChunks The number of chunks in the track
    /// @returns A zero-base table of frame indices and sizes for each chunk index
    func CalculateChunkFrameTable(numChunks uint32)[]ChunkFrames {
        ...
        chunk++ {
            ...
            table[chunk - uint32(1)] = ChunkFrames{
                FirstFrameIndex: firstFrameIndex,
                NumberOfFrames: Samples[lastStscIndex].SamplesPerChunk
            }
