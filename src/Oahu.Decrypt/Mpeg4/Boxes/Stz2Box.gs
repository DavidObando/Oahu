...
open class Stz2Box : FullBox, IStszBox {
    ...
    private init(versionFlags[]uint8, header BoxHeader, parent IBox, sampleSizes List[uint16], fieldSize int32) : base(
        versionFlags,
        header,
        parent
    ) {
    ...
    open override prop RenderSize int64 -> base.RenderSize +
        int64(8) +
        int64(SampleCount * FieldSize / 8) +
        int64(
        (
            if FieldSize == 4 && SampleCount % 2 == 1 {
                1
            } else {
                0
            }
    ...
    protected open override func Render(file Stream) {
        ...
        fieldSize[3] = uint8(
            (
                if shortSpan.AllLessThanOrEqual(uint16(uint8.MaxValue)) {
                    8
                } else {
                    16
                }
