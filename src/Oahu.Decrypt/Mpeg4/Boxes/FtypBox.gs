...
open class FtypBox : Box {
    ...
    shared {
        func Create(majorBrand string, majorVersion int32) FtypBox {
            ArgumentException.ThrowIfNullOrWhiteSpace(majorBrand, "majorBrand")
            return if majorBrand.Length == 4 {
                FtypBox(BoxHeader(16, "ftyp"), majorBrand, majorVersion)
            } else {
                throw ArgumentException("Major brand must be 4 chars long.", "majorBrand")
                default(FtypBox)
            }
        }
    }
