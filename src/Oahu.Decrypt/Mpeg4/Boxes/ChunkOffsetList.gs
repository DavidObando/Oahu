...
class ChunkOffsetList : ICollection[int64] {
    ...
    func Sort() {
        if chunkOffsets64 != nil {
            chunkOffsets64!!.Sort()
            let index = FindLast32bit(CollectionsMarshal.AsSpan(chunkOffsets64))
            if index >= 0 {
    ...
    func GetEnumerator() IEnumerator[int64] -> chunkOffsets32
        .ConvertAll((i uint32) -> int64(i))
        .Concat(chunkOffsets64 ?? List[int64]())
        .GetEnumerator()
    ...
    shared {
        /// Reads 32-bit chunk offsets from the given stream, fixes overflows into 64-bit offsets, and sorts the
        /// offsets.
        ...
        func Read64(file Stream, entryCount uint32) ChunkOffsetList {
            unsafe {
                let pLongs = Marshal.AllocHGlobal(8 * nint(entryCount))
                try {
                    ...
                    longsSpan.Sort()
                    let count32Bit = FindLast32bit(longsSpan) + 1
        ...
        private func FindLast32bit(longsSpan ReadOnlySpan[int64]) int32 {
            ...
            while l <= r {
                ...
                if midValue > int64(uint32.MaxValue) {
                    r = mid - 1
                } else if midValue == int64(uint32.MaxValue) ||
                    mid >= r ||
                    longsSpan[mid + 1] >= int64(uint32.MaxValue) {
                    return mid
                } else {
                    l = mid + 1
                }
