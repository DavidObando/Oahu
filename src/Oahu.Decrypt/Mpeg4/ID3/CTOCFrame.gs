...
class CTOCFrame : Frame {
    init(parent Frame, chapterFlags ChapterFlags, elementId string = "TOC1") : base(
        FrameHeader("CTOC", 0, parent.Version),
        parent
    ) {
    ...
    override prop Size int32 -> Encoding.ASCII.GetByteCount(ElementID) + 3 + ChildElementIDs.Sum(
        (c string) -> Encoding.ASCII.GetByteCount(c) + 1
    ) +
        Children.Sum((c Frame) -> c.Size + c.Header.HeaderSize)
