...
class Id3Tag : Frame {
    private init(file Stream, header Id3Header) : base(header, nil) {
    ...
    override prop Size int32 -> Children.Sum((f Frame) -> f.Size + f.Header.HeaderSize) + EmptyFrame.GetEmptyFrameSize(
        Version
    )
    ...
    shared {
        func Create(file Stream) Id3Tag? {
            let id3Header Id3Header? = Id3Header.Create(file)
            if id3Header == nil || id3Header.Version is not(0x200 or 0x300 or 0x400) {
                return nil
            }
            try {
