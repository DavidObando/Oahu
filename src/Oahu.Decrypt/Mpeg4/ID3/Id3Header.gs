...
class Id3Header : Header {
    ...
    override func Render(stream Stream, renderSize int32, version uint16) {
        stream.Write([]uint8{0x49, 0x44, 0x33}) // "ID3"
        stream.WriteByte(uint8((int32(version) >> 8)))
    ...
    shared {
        func Create(file Stream) Id3Header? {
            try {
                ...
                if bts[0] == uint8('I') && bts[1] == uint8('D') && bts[2] == uint8('3') && (bts[3] is 2 or 3 or 4) {
                    let version = uint16((int32(bts[3]) << 8 | file.ReadByte()))
