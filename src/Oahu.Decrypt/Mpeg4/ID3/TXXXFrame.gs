...
class TXXXFrame : Frame {
    ...
    override prop Size int32 -> if Frame.IsUnicode(FieldName) || Frame.IsUnicode(FieldValue) {
        1 + Frame.UnicodeLength(FieldName) + 2 + Frame.UnicodeLength(FieldValue)
    } else {
    ...
    override func ToString() string -> FieldName
