...
class FrameHeader : Header {
    ...
    override prop HeaderSize int32 -> if Version == uint16(0x200) {
        6
    } else {
        10
    }
    ...
    override func Render(stream Stream, renderSize int32, version uint16) {
        ...
        let size = if version >= uint16(0x400) {
            int32(Header.SyncSafify(renderSize))
        } else {
            renderSize
        }
    ...
    shared {
        func Create(file Stream, version uint16) FrameHeader {
            ...
            if version == uint16(0x200) {
                ...
                let size = (int32(bts2[3]) << 16) | (int32(bts2[4]) << 8) | int32(bts2[5])
                ...
                let size = if version >= uint16(0x400) {
                    Header.UnSyncSafify(Header.ReadUInt32BE(file))
                } else {
                    int32(Header.ReadUInt32BE(file))
                }
