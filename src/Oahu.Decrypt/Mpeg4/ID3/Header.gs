...
open class Header {
    ...
    open override func ToString() string -> if Identifier == "\u0000\u0000\u0000\u0000" {
        "\\0\\0\\0\\0"
    } else {
        (
            if Identifier == "\u0000\u0000\u0000" {
    ...
    shared {
        func ReadUInt16BE(stream Stream) uint16 {
            ...
            return uint16((int32(word[0]) << 8 | int32(word[1])))
        }

        func UnSyncSafify(value uint32) int32 -> int32(
            (
                ((value & uint32(0x7f000000)) >> 3) | ((value & uint32(0x7f0000)) >> 2) | (
                    (value & uint32(0x7f00)) >> 1
                ) | (value & uint32(0x7f))
            )
        )

        func SyncSafify(value int32) uint32 -> uint32(
            (((value << 3) & 0x7f000000) | ((value << 2) & 0x7f0000) | ((value << 1) & 0x7f00) | (value & 0x7F))
        )

        func ReadUInt32BE(stream Stream) uint32 {
            let dword = stackalloc[4]uint8
            stream.ReadExactly(dword)
            return uint32((int32(dword[0]) << 24 | int32(dword[1]) << 16 | int32(dword[2]) << 8 | int32(dword[3])))
        }
        ...
        func WriteUInt32BE(stream Stream, value uint32) -> stream.Write(
            []uint8{uint8((value >> 24)), uint8((value >> 16)), uint8((value >> 8)), uint8(value)}
        )
