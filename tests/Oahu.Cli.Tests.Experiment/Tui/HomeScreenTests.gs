...
type HomeScreenTests class : IDisposable {
    init() {
        Theme.Reset()
    }
    ...
    func CreateScreen(state AppShellState?) HomeScreen {
        ...
        let authFactory func() IAuthService = func() IAuthService { return HSFakeAuthService() }
        let libFactory func() ILibraryService = func() ILibraryService { return HSFakeLibraryService() }
...
type HSRecordingNavigator class : IAppShellNavigator {
    LastModal IModal? = nil
    LastToast string? = nil
    DismissCalled bool = false
    LastBroker TuiCallbackBroker? = nil
    LastTrackedLoad Task? = nil
...
type HSFakeAuthService class : IAuthService {
...
type HSFakeLibraryService class : ILibraryService {
    func ListAsync(filter LibraryFilter?, ct CancellationToken) Task[IReadOnlyList[LibraryItem]] {
        var list List[LibraryItem] = List[LibraryItem]()
        let result IReadOnlyList[LibraryItem] = list
        return Task.FromResult(result)
