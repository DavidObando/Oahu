...
/// `--theme` flag > `NO_COLOR`/`--no-color` > persisted config > default.
@Collection("EnvVarSerial")
class ThemeStartupTests : IDisposable {
    init() {
        Theme.Reset()
    }
    ...
    func ExplicitFlag_Wins_Over_Config_And_NoColor() {
        ...
        let name = TuiCommand.ResolveStartupThemeName(globals, configuredTheme: "HighContrast")
    ...
    func ExplicitFlag_IsCaseInsensitive() {
        ...
        let name = TuiCommand.ResolveStartupThemeName(globals, configuredTheme: nil)
    ...
    func Unknown_Configured_Theme_Falls_Back_To_Default() {
        let globals = GlobalOptions()
        let name = TuiCommand.ResolveStartupThemeName(globals, configuredTheme: "Solarized")
    ...
    func Unknown_Explicit_Override_Falls_Through_To_Config() {
        ...
        let name = TuiCommand.ResolveStartupThemeName(globals, configuredTheme: "Mono")
        Assert.Equal("Mono", name)
    }

    @Fact
    func All_Empty_Returns_Default() {
        let name = TuiCommand.ResolveStartupThemeName(GlobalOptions(), configuredTheme: nil)
        Assert.Equal("Default", name)
    }
}
